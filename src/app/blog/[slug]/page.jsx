...
//   try {
//     const response = await fetch(`${apiBase}/api/blog/posts/${slug}`, { cache: 'no-store' });
...
//     };
//   } catch (e) {
//     return { title: "Blog Post | Microters" };
//   }
// }
...
//   const apiBase = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3000';
//   const res = await fetch(`${apiBase}/api/blog/posts/${slug}`, { cache: 'no-store' });
...
//           </div>
//         </div>
//         {/* 2. FEATURED IMAGE (UPDATED DESIGN) */}
//        <div>
//            {post.featuredImage && (
...
//                 <div className="relative aspect-21/9 rounded-4xl overflow-hidden shadow-lg border border-white/50 bg-gray-50">
//                 <Image 
...
//               {/* Company CTA Block  */}
//               <div 
//                 className="mt-12 relative rounded-2xl overflow-hidden px-8 py-12 bg-linear-to-r from-[#FF9A3E] to-[#FF5266] text-white shadow-lg text-center"
//               >
...
//                     <Link 
//                       href="/contact" 
//                       className="inline-flex items-center gap-2 bg-white text-[#f35d36] px-8 py-3 rounded-lg font-bold hover:shadow-lg transition-all hover:scale-105"
//                     >
...
//                    <Link 
//                      href="/services" 
//                      className="block w-full py-2 rounded bg-[#f35d36] text-white text-xs font-bold hover:bg-[#d84e2a] transition-colors"
//                    >
//                       View Services
//                    </Link>
//                 </div>
...
import { notFound } from 'next/navigation';
import * as cheerio from 'cheerio';
// 👇 FIX 1: Added FiFolder to imports
...
// --- Helper: Direct DB Fetch (Fixes Vercel 500 Error) ---
...
export async function generateMetadata({ params }) {
  const { slug } = await params;
  const post = await getPostData(slug); // Uses direct DB call
...
export default async function BlogPostPage({ params }) {
  ...
  return (
    <>
      {/* 1. HERO SECTION */}
      {/* Added pb-32 to give space for the image to overlap below */}
      <section className="relative pt-32 pb-32 lg:pt-44 lg:pb-48 overflow-hidden bg-[#FFFBF8]">
        ...
        <div className="container mx-auto px-4 py-16 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <main className="lg:col-span-7 order-1 lg:order-2">
              ...
              <div className="mt-12 relative rounded-2xl overflow-hidden px-8 py-12 bg-gradient-to-r from-[#FF9A3E] to-[#FF5266] text-white shadow-lg text-center">
                <div className="relative z-10 flex flex-col items-center justify-center gap-4">
                    <h3 className="text-2xl md:text-3xl font-bold">Grow with Microters?</h3>
                    <p className="opacity-90 max-w-md mx-auto mb-4">
                      Let's discuss your next project together. We help businesses scale with proven strategies.
