...
const SentenceExpanderComponent = () => {
  ...
  const handleGenerateResults = async () => {
    // Validation
    if (!topic.trim()) {
      toast.warning("Please write a sentence to expand.");
      return;
    }
    if (topic.length > 500) {
      toast.warning("The sentence must not exceed 500 characters.");
      return;
    }
    if (!country) {
    ...
    try {
      ...
      const body = {
        messages: [
          {
            role: "system",
            content: `You are an assistant that generates expanded versions of sentences. Language: ${language}. Tone: ${selectedTone}. Ensure output is clear and engaging.`,
          },
          {
            role: "user",
            content: `Expand this sentence: "${topic}". Provide 6 distinct expanded versions. Return them separated by "###".`,
          },
        ],
        temperature: 0.7,
        max_tokens: 1000,
  ...
  return (
    <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-5 lg:p-8 mb-8 border border-slate-200 shadow-lg shadow-[#ff9a3e]/10">
      ...
      <div className="space-y-8">
        {/* Input Area */}
        <LegendWrapper label="Original Sentence" required>
          <textarea
        ...
        >
          {loading ? (
            ...
            <>
              <FaWandMagicSparkles /> Expand Sentence
            </>
          )}
        </button>
      </div>
