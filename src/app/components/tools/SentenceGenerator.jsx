...
const SentenceGeneratorComponent = () => {
  ...
  const handleGenerateResults = async () => {
    // Validation
    if (!topic.trim()) {
      toast.warning("Please provide a topic or keyword.");
      return;
    }
    if (topic.length > 500) {
    ...
    try {
      // 1. Verify Recaptcha
      const token = await executeRecaptcha("generate_sentence");
      ...
      const body = {
        messages: [
          {
            role: "system",
            content: `You are an assistant that generates 10 SEO-friendly sentences for the topic provided. Sentences should be written in ${language} with a ${selectedTone} tone.`,
          },
          {
            role: "user",
            content: `Generate exactly 10 distinct and meaningful SEO-friendly sentences based on this topic: "${topic}". Ensure each sentence is unique and captures attention. Return the sentences separated by "###".`,
          },
        ],
        temperature: 0.7,
        max_tokens: 1000,
  ...
  return (
    <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-5 lg:p-8 mb-8 border border-slate-200 shadow-lg shadow-[#ff9a3e]/10">
      ...
      <div className="space-y-8">
        {/* Input Area */}
        <LegendWrapper label="Topic / Keyword" required>
          <textarea
        ...
        >
          {loading ? (
            ...
            <>
              <FaWandMagicSparkles /> Generate Sentences
            </>
          )}
        </button>
      </div>
        ...
        <div className="mt-12 border-t border-gray-100 pt-10">
          ...
          <div className="grid grid-cols-1 gap-4">
            {results.map((res, idx) => (
              <div
                key={idx}
                className="group bg-gray-50 p-6 rounded-xl border border-gray-200 hover:border-[#f35d36] transition-all relative flex items-start gap-4"
              >
                <div className="w-8 h-8 bg-[#feefeb] text-[#f35d36] rounded-full flex items-center justify-center font-bold text-sm shrink-0">
                  {idx + 1}
                </div>
                <div className="grow pt-1 pr-8">
