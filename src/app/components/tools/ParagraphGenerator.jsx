...
const toneOptions = [
  ...
  { label: "Middle School", value: "middle-school", icon: <FaBook /> },
  { label: "High School", value: "high-school", icon: <FaBookOpen /> },
  ...
  { label: "Persuasive", value: "persuasive", icon: <FaBrain /> },
...
const ParagraphGenerator = () => {
  ...
  const handleGenerateResults = async () => {
    // Validations
    if (!topic.trim()) {
      toast.warning("Please write a topic before generating paragraphs.");
      return;
    }
    if (topic.length > 500) {
    ...
    const body = {
      messages: [
        ...
        {
          role: "user",
          content: `Generate three distinct and meaningful Paragraphs based on the following Topic: "${topic}". Separate the paragraphs with "###".`,
        },
      ],
      temperature: 0.7,
      max_tokens: 1400,
    ...
    try {
      ...
      toast.error("Failed to generate paragraphs. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-5 lg:p-8 mb-8 border border-slate-200 shadow-lg shadow-[#ff9a3e]/10">
        <div className="space-y-8">
          {/* Topic Input */}
          <LegendWrapper label="Paragraph Topic" required>
            <textarea
          ...
          >
            {loading ? (
              <span className="animate-pulse">Generating Paragraphs...</span>
            ) : (
              <>
                <FaWandMagicSparkles /> Generate Paragraphs
              </>
            )}
          </button>
        </div>
