...
const ThesisStatementGeneratorComponent = () => {
  ...
  const handleGenerateResults = async () => {
    ...
    if (!targetAudience.trim()) {
      toast.warning("Please provide a target audience.");
      return;
    }
    if (!country) {
    ...
    try {
      ...
      const body = {
        messages: [
          {
            role: "system",
            content: `You are an assistant that generates well-structured thesis statements for essays, research papers, or projects. Each thesis statement should be concise, clear, and tailored to the given topic, target audience, tone, and language.`,
          },
          {
            role: "user",
            content: `Generate three distinct thesis statements for the topic "${topic}" with targeting "${targetAudience}". Ensure the statements are written in ${language} with a ${selectedTone} tone. Separate the thesis statements with "###".`,
          },
        ],
        temperature: 0.7,
        max_tokens: 1000,
  ...
  return (
    <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-5 lg:p-8 mb-8 border border-slate-200 shadow-lg shadow-[#ff9a3e]/10">
      ...
      <div className="space-y-8">
        ...
        <LegendWrapper label="Target Audience" required>
          <input
            type="text"
            className={baseInputStyles}
            placeholder="e.g. Business Owners, University Students..."
            ...
            {results.map((res, idx) => (
              ...
              >
                ...
                <div className="pl-10 pt-1">
                  <p className="text-gray-700 text-base leading-relaxed font-serif font-medium">
                    {res}
                  </p>
                </div>
                <button
